...
const keynotesObject = {
    title: 'Node.js introduction',
    order: 1,
    uri:   'https://lectrum.io/keynotes/lesson-1',
};

describe('should handle secure add route lessons/keynotes by hash', () => {
    ...
    test('should have POST 200 status code for corret data', async (done) => {
        const response = await server.post('/lessons/123/keynotes').send(keynotesObject);

        expect(response.statusCode).toBe(204);
        done();
    });
    test('should return nothing', async (done) => {
        const response = await server.put('/lessons/123/keynotes').send(keynotesObject);
